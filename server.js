...
const FileStore = require('connect-session-file-store')(session);
...
app.use(session({
  ...
  secret: process.env.SESSION_SECRET || 'tindarhan-pos-secret-key-change-in-production',
  resave: false,
...
app.post('/api/login', (req, res) => {
  ...
  const passwordMatch = bcrypt.compareSync(String(password), user.passwordHash);
  if (!passwordMatch) {
...
app.get('/api/session', (req, res) => {
  ...
  console.log('[SESSION] Check - loggedIn:', isLoggedIn, 'user:', req.session?.user || 'none');
...
app.get('/api/state', requireAuth, (req, res) => {
  try {
    console.log('[STATE] Loading state for user:', req.session.user);
    const data = readData();
    console.log('[STATE] Loaded:', data.items.length, 'items,', data.sales.length, 'sales');
